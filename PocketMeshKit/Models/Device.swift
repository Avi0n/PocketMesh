...
public extension Device {
    /// Updates device from DeviceInfo response
    func update(from info: DeviceInfo) {
        self.firmwareVersion = info.firmwareVersion
        self.firmwareVersionString = info.firmwareVersionString
        self.manufacturerName = info.manufacturerName
        self.buildDate = info.buildDate
        self.maxContacts = info.maxContacts
        self.maxChannels = info.maxChannels
        self.blePin = info.blePin
    }

    /// Updates device from SelfInfo response
    func update(from info: SelfInfo) {
        ...
        self.spreadingFactor = info.spreadingFactor
        self.codingRate = info.codingRate
        self.multiAcks = info.multiAcks > 0
        self.advertLocationPolicy = info.advertLocationPolicy.rawValue
        self.manualAddContacts = info.manualAddContacts > 0
        self.telemetryModeBase = info.telemetryModes & 0x03
        self.telemetryModeLoc = (info.telemetryModes >> 2) & 0x03
        self.telemetryModeEnv = (info.telemetryModes >> 4) & 0x03
    }
