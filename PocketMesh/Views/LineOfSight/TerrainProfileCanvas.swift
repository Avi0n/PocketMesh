...
struct TerrainProfileCanvas: View {
    ...
    private let fresnelClear = Color.teal.opacity(0.35)
    private let fresnelObstructed = Color.orange.opacity(0.5)
    ...
    private var legendView: some View {
        HStack(spacing: 16) {
            ...
            legendItem(color: fresnelClear, label: "Clear")
...
extension TerrainProfileCanvas {
    ...
    private func drawFresnelZone(context: GraphicsContext, coords: ChartCoordinateSpace) {
        ...
        let segments = FresnelZoneRenderer.findSegments(samples: profileSamples)
    ...
    private func drawFresnelSegment(
        context: GraphicsContext,
        coords: ChartCoordinateSpace,
        segment: FresnelSegment
    ) {
        // Filter samples within this segment
        let segmentSamples = profileSamples.filter { sample in
            sample.x >= segment.xStart && sample.x <= segment.xEnd
        }

        guard segmentSamples.count >= 2 else { return }

        var path = Path()

        // Walk left-to-right building top edge (always yTop)
        if let first = segmentSamples.first {
            path.move(to: coords.point(x: first.x, y: first.yTop))
        }
        for sample in segmentSamples.dropFirst() {
            path.addLine(to: coords.point(x: sample.x, y: sample.yTop))
        }

        // Walk right-to-left building bottom edge (clamped)
        for sample in segmentSamples.reversed() {
            path.addLine(to: coords.point(x: sample.x, y: sample.yVisibleBottom))
        }

        path.closeSubpath()

        let fillColor = segment.isObstructed ? fresnelObstructed : fresnelClear
