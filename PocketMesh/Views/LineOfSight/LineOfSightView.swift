...
private let analysisSheetDetentExpanded: PresentationDetent = .fraction(0.9)
...
struct LineOfSightView: View {
    ...
    private var terrainProfileSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                if case .result(let result) = viewModel.analysisStatus {
                    Label(
                        "Adjusted for earth curvature (\(LOSFormatters.formatKFactor(result.refractionK)))",
                        systemImage: "globe"
                    )
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                } else {
                    Label("Adjusted for earth curvature", systemImage: "globe")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            VStack(spacing: 12) {
                ...
                HStack {
                    ...
                    TextField("MHz", value: Binding(
                        get: { viewModel.frequencyMHz },
                        set: { viewModel.frequencyMHz = $0 }
    ...
    private func zoomToShowBothPoints() {
        ...
        // Add padding: more on bottom for sheet, some on sides
        let latDelta = max(0.01, (maxLat - minLat) * 2.0)
        let lonDelta = max(0.01, (maxLon - minLon) * 1.5)

        // Offset center slightly upward to account for sheet covering bottom half
        let offsetCenterLat = centerLat + (latDelta * 0.15)

        let center = CLLocationCoordinate2D(latitude: offsetCenterLat, longitude: centerLon)
        let span = MKCoordinateSpan(latitudeDelta: latDelta, longitudeDelta: lonDelta)
        let region = MKCoordinateRegion(center: center, span: span)
