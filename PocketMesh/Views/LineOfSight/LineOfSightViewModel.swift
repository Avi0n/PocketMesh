...
enum AnalysisStatus: Equatable {
    ...
    static func == (lhs: AnalysisStatus, rhs: AnalysisStatus) -> Bool {
        ...
        case (.result(let lhsResult), .result(let rhsResult)):
            return lhsResult.distanceMeters == rhsResult.distanceMeters
                && lhsResult.totalPathLoss == rhsResult.totalPathLoss
                && lhsResult.clearanceStatus == rhsResult.clearanceStatus
    ...
    func analyze() {
        ...
        analysisTask = Task { @MainActor in
            do {
                ...
                let distance = RFCalculator.distance(
                    from: pointA.coordinate,
                    to: pointB.coordinate
                )
                ...
                let result = RFCalculator.analyzePath(
                    elevationProfile: profile,
                    pointAHeightMeters: Double(pointA.additionalHeight),
                    pointBHeightMeters: Double(pointB.additionalHeight),
